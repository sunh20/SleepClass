function errorbar_sig(mean1,mean2,ste1,ste2,hyp,pval)
% plots the error bars and significance
% *pval = 0.05
% **pval = 0.01
% ***pval = 0.001
...
X = 1:length(mean1);
X = [X'-gap,X'+gap];
errorbar(X,[mean1 mean2],[ste1 ste2],'k.')

% sigstar
for idx = 1:5
    if hyp(idx) == 1
        sigstar({[idx-0.2 idx+0.2]},pval(idx))
    end
end
